...
const columns = [
    ...
    {
        key: 'institution',
        label: __('classrooms.col_institution'),
        sortable: false,
    },
...
// Teachers/subjects filtered by the chosen institution.
const availableTeachers = computed(() =>
    props.teachers.filter(
        (t) => String(t.institution_id) === String(form.institution_id),
    ),
);
const availableSubjects = computed(() =>
    props.subjects.filter(
        (s) => String(s.institution_id) === String(form.institution_id),
    ),
);
...
const openEdit = (classroom) => {
    ...
    form.institution_id = classroom.institution_id || '';
...
const submit = () => {
    ...
    if (isEditing.value) {
        form.put(route('super-admin.classrooms.update', selectedId.value), {
            preserveScroll: true,
            onSuccess,
        });
    } else {
        form.post(route('super-admin.classrooms.store'), {
            preserveScroll: true,
            onSuccess,
        });
    }
...
const confirmDelete = (classroom) => {
    triggerConfirm(
        ...
        () =>
            router.delete(
                route('super-admin.classrooms.destroy', classroom.id),
                { preserveScroll: true },
            ),
    );
};
...
const confirmToggle = (classroom) => {
    ...
    triggerConfirm(
        ...
        () =>
            router.post(
                route('super-admin.classrooms.toggle', classroom.id),
                {},
                { preserveScroll: true },
            ),
    );
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <PageHeader :title="__('classrooms.super_header')">
                <template #actions>
                    <button
                        @click="openCreate"
                ...
                <div>
                    ...
                    >
                        <option value="" disabled>
                            {{ __('classrooms.form_select_institution') }}
                        </option>
                        <option
