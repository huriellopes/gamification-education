        ...
        <div class="py-12 bg-zinc-950 min-h-[calc(100vh-64px)] text-zinc-100">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-8">
                ...
                <div v-if="isFormOpen" class="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6 shadow-xl backdrop-blur-md">
                    <h3 class="text-lg font-bold text-white mb-4">Cadastrar Nova Instituição</h3>
                    <form @submit.prevent="submit" class="space-y-4">
                        <div>
                            <label for="name" class="block text-xs font-bold uppercase tracking-wider text-zinc-400 mb-1">Nome da Instituição</label>
                            <input
                                id="name"
                                v-model="form.name"
                                type="text"
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    >
                        <h4 class="text-lg font-bold text-white">{{ inst.name }}</h4>
                        <p class="mt-2 text-sm text-zinc-400 line-clamp-3 min-h-[60px]">
                            {{ inst.description || 'Sem descrição cadastrada.' }}
                        </p>
                        
                        <div class="mt-6 flex items-center justify-between border-t border-zinc-850 pt-4 text-xs">
                            <div class="flex items-center gap-1 text-zinc-500">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-4 w-4 text-indigo-400">
                                ...
                                <span>Alunos: <strong>{{ inst.users_count || 0 }}</strong></span>
                            </div>
                            <div class="flex items-center gap-1 text-zinc-500">
                                ...
                                <span>Matérias: <strong>{{ inst.subjects_count || 0 }}</strong></span>
                            </div>
                        </div>
                    </div>

                    <div v-if="institutions.length === 0" class="col-span-full rounded-2xl border border-dashed border-zinc-800 p-12 text-center text-zinc-500">
                        Nenhuma instituição de ensino cadastrada ainda. Clique em "Nova Instituição" para começar!
                    </div>
                </div>
