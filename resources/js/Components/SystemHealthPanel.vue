...
const checkLabel = (key) => __(`superadmin.health.checks.${key}.label`);

const checkDescription = (check) => {
    if (check.status === 'ok') {
        return __(`superadmin.health.checks.${check.key}.ok`);
    }

    return `${check.value} ${__(`superadmin.health.checks.${check.key}.problem`)}`;
};

const statusLabel = (status) => __(`superadmin.health.status.${status}`);
</script>
...
<template>
    ...
    >
        ...
        >
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    >
                        {{ __('superadmin.health.title') }}
                    </h3>
                    <p class="text-xs text-zinc-400">
                        {{ __('superadmin.health.subtitle') }}
                    </p>
                </div>
            </div>
            ...
            <!-- Pílula de resumo -->
            <div
                class="inline-flex items-center gap-2 self-start rounded-full border px-3.5 py-1.5 text-xs font-bold"
                :class="[
                    classesFor(summary.status).ring,
                    classesFor(summary.status).soft,
            ...
            >
                ...
                <span v-if="isHealthy">{{
                    __('superadmin.health.all_healthy')
                }}</span>
                <span v-else>
                    {{ summary.alerts }} {{ __('superadmin.health.alerts_label') }}
                    <template v-if="summary.critical > 0">
                        · {{ summary.critical }}
                        {{ __('superadmin.health.critical_label') }}
                    </template>
                </span>
            </div>
        </div>
        ...
        <div
            v-if="isHealthy"
            class="flex items-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 px-4 py-3"
        >
            <CheckCircle2 class="h-5 w-5 shrink-0 text-emerald-400" />
            <p class="text-sm text-zinc-300">
                {{ __('superadmin.health.all_healthy_detail') }}
            </p>
        </div>
